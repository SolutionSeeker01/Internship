...
### 5.15 Order Manager & Position Management Specification

**Responsibility:** Manage post-submission order lifecycles, fill notifications, target quantity splitting, limit-only exit orders, OCO cancellation, trailing stop-loss adjustments, and position PnL accounting.
**Status:** Build in V1.
**Inputs:** `orders` records (status `SUBMITTED`/`OPEN`), broker WebSocket order updates, LTP market data feeds.

#### 1. Separation of Responsibilities
- **Strategy Engine:** Produces the trade generation parameters (`action`, `entry`, `stoploss`, `t1`, `t2`, `t3`).
- **Order Manager:** Governs how the open position is managed across its lifecycle (quantity splitting, limit-only exit orders, trailing stop calculations, OCO cancellations).

#### 2. Position Target Quantity Split Policy
When the entry order fills (`filled_quantity` $Q$), child target orders are created as:
- **TP1 Quantity (50%):** $\lfloor Q \times 0.50 \rfloor$
- **TP2 Quantity (25%):** $\lfloor Q \times 0.25 \rfloor$ (Integer floor value)
- **TP3 Quantity (25% / Remaining):** $Q - (\text{TP1 Qty} + \text{TP2 Qty})$ (Remaining quantity)

**Worked Examples:**
- **Example A (Total Quantity = 90):**
  - TP1 (50%): $\lfloor 90 \times 0.50 \rfloor = \mathbf{45}$
  - TP2 (25%): $\lfloor 90 \times 0.25 \rfloor = \mathbf{22}$
  - TP3 (Remaining): $90 - (45 + 22) = \mathbf{23}$
- **Example B (Total Quantity = 100):**
  - TP1 (50%): $\lfloor 100 \times 0.50 \rfloor = \mathbf{50}$
  - TP2 (25%): $\lfloor 100 \times 0.25 \rfloor = \mathbf{25}$
  - TP3 (Remaining): $100 - (50 + 25) = \mathbf{25}$

#### 3. Limit-Only Exit Policy & Broker Adapter Handoff (Principle P1 & P4)
- **Validation Ownership:** The maximum 5% Stop-Loss distance check belongs exclusively to **Signal Validation** before signals enter the system. The Order Manager performs zero re-validation of signal prices (Principle **P1**).
- **Business Intent Handoff:** The Order Manager requests `LIMIT` exit orders as a non-negotiable business requirement for all TP, Stop-Loss, and Trailing Stop exits.
- **Broker Adapter Transport Payload Responsibility (Principle P4):** Absolute market exit orders are strictly forbidden for SL and TP exits. The concrete Broker Adapter constructs the broker-specific API payload required to satisfy this business rule (setting `trigger_price`, `price`, limit offset buffers, `variety`, `product`, `validity`, and any broker-specific fields) to guarantee execution without violating broker API requirements.



#### 4. Trailing Stop-Loss Rules & Formulas
- **Activation Threshold:** Trailing stop logic activates when live LTP reaches **70% of distance from Entry to T1**:
...
- **Trailing Stop Formulas:**
  - **BUY Trailing SL:**
    $$\text{New SL} = \text{Original SL} + \left(0.50 \times (\text{T1} - \text{Entry})\right) + \left(0.25 \times (\text{LTP} - \text{Entry})\right)$$
  - **SELL Trailing SL:**
    $$\text{New SL} = \text{Original SL} - \left(0.50 \times (\text{Entry} - \text{T1})\right) - \left(0.25 \times (\text{Entry} - \text{LTP})\right)$$

- **Trailing Exit Conditions:**
  - **BUY:** If $\text{LTP} < \text{Trailing SL} \implies$ Submit Limit Exit order for remaining open quantity.
  - **SELL:** If $\text{LTP} > \text{Trailing SL} \implies$ Submit Limit Exit order for remaining open quantity.

#### 5. Partial Booking & OCO Management
- When **TP1** or **TP2** fills, Order Manager executes OCO adjustment: updates remaining open quantity ($Q_{\text{rem}} = Q - \text{Executed Partial Qty}$) and updates the Stop-Loss order quantity to match $Q_{\text{rem}}$.
- Trailing stop calculation continues dynamically on the remaining open quantity ($Q_{\text{rem}}$) until the trade is fully closed.

#### 6. Explicit Trade Closure Rule
- **Rule:** If TP3 executes, or the remaining open quantity reaches zero (whether through TP3, Stop-Loss, or Trailing Stop exit), the trade shall transition to `CLOSED` status in the `trades` table.
- Any remaining child orders (if applicable) shall be cancelled via OCO, and realized P&L shall be finalized.
